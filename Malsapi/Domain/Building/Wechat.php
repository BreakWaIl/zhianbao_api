...
class Domain_Building_Wechat {
    ...
    public function getByShopId($shopId,$cols = '*'){
        $model = new Model_Zhianbao_Wechat ();
        $rs = $model->getByWhere( array('shop_id'=>$shopId), $cols );
        if (! $rs)
        return false;
        return $rs;
    }
    ...
    public function addWechatMenu($appid,$wechatId,$content,$userId,$shopId){
        ...
        if(isset($isOpen) && $isOpen['shop_value'] == 'open'){
            $accessToken = $this->getAccessToken($appid,$wechatId);
    ...
    public function delWechatMenu($appid,$wechatId,$userId,$shopId){
        //关闭微信自定义菜单
        $domainShopKV = new Domain_Shenpu_ShopKvStore($shopId, $userId);
    ...
    public function massWechatMsg($userId , $appid , $wechatId , $msgType , $textContent = '' , $mediaId = ''){
        ...
        $result = json_decode($result,true);
        if($result['errcode'] == 0){
            $masslogModel = new Model_Shenpu_WechatMassLog();
            $log = array(
				'user_id' => $userId,
				'wechat_id' => $wechatId,
    ...
    /*public function addWechatDeleteInfo($userWechat) {
     $model = new Model_Shenpu_WechatDelete();
     $rs = $model->insert ( $userWechat );
    ...
    public function getPayOpenid($userId,$shopId){
        ...
        if((! isset($info)) || (isset($info) && $info['shop_value'] == 'open')) {
            //开启了代收款
            $agentInfo = DI ()->cookie->get('sp_agent');
    ...
    //开启微信自定义菜单
    public function openWechatMenu($appid,$wechatId,$userId,$shopId){
        $domainShopKV = new Domain_Shenpu_ShopKvStore($shopId, $userId);
        $domainShopKV->set('WechatMenuSwitch','open');
        $info = $domainShopKV->get('WechatMenu');
        if(! empty($info['shop_value'])) {
           $menuContent = json_decode($info,true);
            $rs = $this->addWechatMenu($appid,$wechatId,$menuContent,$userId,$shopId);
            if( ! $rs){
                return false;
            }
        }else{
    ...
    //获取本地自定义菜单
    public function getLocalWechatMenu($appid,$wechatId,$userId,$shopId){
        $domainShopKV = new Domain_Shenpu_ShopKvStore($shopId, $userId);
        $info = $domainShopKV->get('WechatMenu');
        $swtich = $domainShopKV->get('WechatMenuSwitch');
        if(isset($swtich) && $swtich['shop_value'] == 'open'){
            $open = true;
        }else{
            $open = false;
        }
        if($info) {
            //本地存放了自定义菜单
            $menuInfo = json_decode($info['shop_value'],true);
    ...
    public function sendTempMsg($appid,$wechatId,$type,$tempData){
        ...
        */
        ...
        $templatModel = new Model_Shenpu_WechatTemp();
