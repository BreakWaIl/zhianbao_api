 ...
 * @author: Dm
 */
class Api_Jiafubao_Company_HouseStaff_Delete extends PhalApi_Api {
  ...
  /**
    ...
    public function Go() {
        ...
        //判断是否在使用
        $isUser = $houseStaffDomain->isUser($this->staffId);
        if(!empty($isUser)){
            $rs['code'] = 137;
            $rs['msg'] = T('Being used');
            return $rs;
...
//        $res = $houseStaffDomain->deleteHouseStaff($this->staffId);
        $res = 0;
        if( $res){
            $status = 0;
        }else{
